...
<form class="layui-form layui-form-pane" action="" id="saveForm">
  ...
  <div class="layui-form-item">
   ...
   <div class="layui-form-item">
  		<label class="layui-form-label">视频类型<span>*</span></label></label>
	    <div class="layui-input-block">
	    	<select name="video_type" id="video_type" required="required" >
	    		<c:forEach items="${ hyList}" var="hy">
	    			<option value="${hy.param_value }" ${hy.param_value ==  informationMap.video_type  ? 'selected' : '' } >${hy.param_name} </option>
	    		</c:forEach>
	    	</select>
	    </div>
  </div>
		...
		layui.config({
		   ...
		   upload.render({
		    ...
		    form.on('submit(saveSubmit)', function(data){
				...
				webplus.doAjax('<%=basePath%>videomanage/videotype/updateInformation',data.field,'','','1')
				//阻止表单默认提交
			    return false;
		 	});
