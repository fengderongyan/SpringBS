...
<form class="layui-form layui-form-pane" action="" id="saveForm">
  <div class="layui-form-item">
	    ...
	    <div class="layui-input-block">
	    	...
	    	<input type="text" name="name" lay-verify="required" value="${informationMap.name }"  autocomplete="off"  class="layui-input">
	    </div>
  </div>
  <div class="layui-form-item">
  		<label class="layui-form-label">性别<span>*</span></label>
	    <div class="layui-input-block">
		    <input type="radio" name="sex" value="1" title="男" <c:if test="${informationMap.sex == 1 }">checked</c:if>>
	      	<input type="radio" name="sex" value="2" title="女" <c:if test="${informationMap.sex == 2 }">checked</c:if>>
	    </div>
  </div>
  <div class="layui-form-item">
  		<label class="layui-form-label">简介<span>*</span></label></label>
	    <div class="layui-input-block">
	    	<input type="text" name="introduction" value="${informationMap.introduce }" lay-verify="required"  autocomplete="off"  class="layui-input">
	    </div>
  </div>
  <div class="layui-form-item">
  		<label class="layui-form-label">排序</label>
	    <div class="layui-input-block">
	      <input type="number" name="order_num" value="${informationMap.order_num }"  autocomplete="off"  class="layui-input">
	    </div>
  </div>
  <div class="layui-form-item" >
	  <div class="layui-upload-list" id="lay-img">
	    ...
	    <img class="layui-upload-img" id="photo_img" src="${informationMap.photo }" width="70px">
	    <input type="hidden" id="photo" name="photo" value="${informationMap.photo }">
	  </div>
  </div>
		...
		layui.config({
		   ...
		   upload.render({
		    ...
		    form.on('submit(saveSubmit)', function(data){
				...
				webplus.doAjax('<%=basePath%>videomanage/expert/updateInformation',data.field,'','','1')
				//阻止表单默认提交
			    return false;
		 	});
