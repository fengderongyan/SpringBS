...
<div type="text/html" id="toolbar" style="display:none" >
  <c:if test="${QX.add == 1}">
	  <div class="layui-btn-container">
  		<button class="layui-btn layui-btn-sm" lay-event="addInformation"><i class="layui-icon">&#xe654;</i>新增首页资讯</button>
	  </div>
  </c:if>
</div>
...
<script>
	layui.config({
		...
		webplus.init('<%=basePath%>informationmanage/getInformaticaManageList',cols);
		 //行单击事件
		    ...
		    switch(obj.event){
		    	case 'show':
		    	   ...
		    	   if(webplus.checkRowEditMode(obj)){
		    		 var title = '首页资讯详情';
		    		 var url = '<%=basePath%>informationmanage/editInformationManager?id='+ data.id;
		    		 var w = '900';
		    		 var h = '550';
		    	...
		    	case 'del':
		    		if(webplus.checkRowEditMode(obj)){
		    			...
		    			webplus.doAjax('<%=basePath%>informationmanage/delInformation?id=' + data.id ,params,'确定删除吗？','','0');//0：刷新当前页面，1：刷新父级页面
		    		}
		    		break;
		    }
		  });
		 ...
		 //监听表头按钮事件
		  table.on('toolbar(dataList)', function(obj){
		    ...
		    switch(obj.event){
		      	case 'addInformation':
			        //添加资讯信息
			        webplus.openWindowBase('<%=basePath%>informationmanage/addInformation','新增首页资讯信息','900','550');
	     	    break;
		    }
		  });	
